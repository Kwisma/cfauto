    ...
    async fetch(request, env) {
        try {
            ...
            if (correctCode && !cookieHeader.includes(`auth=${correctCode}`)) {
                return await publicAccountList(env);
            }
...
// ================= 后端辅助函数 =================
async function publicAccountList(env) {
    const ACCOUNTS_KEY = 'ACCOUNTS_UNIFIED_STORAGE';

    let accounts = JSON.parse((await env.CONFIG_KV.get(ACCOUNTS_KEY)) || '[]');

    const stats = await fetchInternalStats(accounts);

    accounts = accounts.map((acc) => {
        const stat = stats.find((s) => s.alias === acc.alias);

        return {
            ...acc,
            stats: stat && !stat.error ? stat : { total: 0, max: 100000 },
        };
    });
    ...
    const rows = accounts
        .map((acc) => {
            const total = acc.stats?.total || 0;
            const max = acc.stats?.max || 100000;
